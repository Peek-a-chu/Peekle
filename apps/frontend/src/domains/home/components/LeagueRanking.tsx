      ...
      <div className="flex-1 pr-1 -mr-1">
        <div className="space-y-0.5 relative">
          {data.members.map((member, index) => {
            ...
            const showPromoteDivider =
              member.status === 'PROMOTE' && nextMember?.status !== 'PROMOTE';
            ...
            const showDemoteDivider2 =
              member.status === 'PROMOTE' && nextMember?.status === 'DEMOTE';
              ...
              <div key={member.rank}>
                ...
                >
                  ...
                  <div className="w-6 flex justify-center shrink-0">
                    <span
                      className={`text-xs font-medium ${member.me ? 'text-primary font-bold' : 'text-muted-foreground'}`}
                    >
                      {member.rank}
                    </span>
                  </div>
                  ...
                  <div className="flex-1 flex items-center gap-2 ml-2 overflow-hidden">
                    <div
                      className={`w-6 h-6 rounded-full overflow-hidden shrink-0 border ${member.me ? 'border-primary/30' : 'border-border'}`}
                    >
                    ...
                    <span
                      className={`text-xs font-medium truncate ${member.me ? 'text-primary font-bold' : 'text-foreground'}`}
                    >
                      {member.name}
                      {member.me && (
                  ...
                  {/* 점수 */}
                  <span
                    className={`text-xs font-bold ml-1 ${member.me ? 'text-primary' : 'text-muted-foreground'}`}
                  >
