...
import { CheckCircle2, Circle, ChevronLeft, ChevronRight, Trophy, User, ExternalLink } from 'lucide-react'
...
function SolvedByList({
    ...
    solvedBy?: { id: string; nickname: string; team?: Team }[]
    teamType: 'INDIVIDUAL' | 'TEAM'
    participants: GamePlayParticipant[]
}) {
            ...
            <div className="mt-2 flex items-center justify-between text-xs">
                    ...
                    <div className="flex items-center gap-1 h-5">
                        {blueSolved.length > 0 ? (
                            <div className="flex -space-x-1.5">
                                {blueSolved.map((s) => renderAvatar(s.id))}
                            </div>
                        ) : (
                ...
                <div className="flex flex-col gap-1 items-end">
                    ...
                    <div className="flex items-center gap-1 h-5">
                        {redSolved.length > 0 ? (
                            <div className="flex -space-x-1.5">
                                {redSolved.map((s) => renderAvatar(s.id))}
                            </div>
                        ) : (
    ...
    return (
        <div className="mt-2 flex items-center gap-2">
            <div className="flex -space-x-1.5">
                {solvedBy.slice(0, 5).map((s) => renderAvatar(s.id))}
            </div>
            {solvedBy.length > 5 && (
