...
export function CCTeamCard({ team, teamColor, isWinner, mvpId, score, mode }: CCTeamCardProps) {
    const theme = teamColor === 'RED'
        ? { bg: 'bg-red-500/5', border: 'border-red-500/20', text: 'text-red-500', glow: 'shadow-red-500/20' }
        : { bg: 'bg-blue-500/5', border: 'border-blue-500/20', text: 'text-blue-500', glow: 'shadow-blue-500/20' };
    ...
    return (
        <div className={`flex-1 flex flex-col gap-3 p-4 rounded-2xl border ${theme.border} ${theme.bg} relative ${isWinner ? 'z-10 shadow-2xl animate-slam-effect ' + theme.glow : 'transition-all duration-500 scale-95 opacity-80 grayscale-[0.3]'}`}>
            {/* Winner Stamp */}
            {isWinner && (
                    ...
                    <div className="transform -rotate-[30deg]">
                        ...
                        >
                            <div className="text-center">
                                <span className="text-4xl font-black tracking-widest uppercase drop-shadow-sm block">WINNER</span>
                                <span className="text-xs font-bold tracking-[0.3em] uppercase opacity-70 block mt-1">VICTORY</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className={`flex items-center justify-between pb-4 border-b ${theme.border} mb-2`}>
                ...
                <div className="text-right">
                    <span className="text-xs font-bold text-muted-foreground block uppercase">Total Solved</span>
                    ...
                    >
                        ...
                        <div className="flex-1 flex items-center gap-3 overflow-hidden">
                            <div className="w-8 h-8 rounded-full bg-muted overflow-hidden border border-border/50 shrink-0">
                                {p.profileImg && (
                                    <Image src={p.profileImg} alt={p.nickname} width={32} height={32} className="object-cover" />
                                )}
                            </div>
                            <div className="flex flex-col min-w-0">
                                <span className={`text-sm font-bold truncate ${p.isMe ? theme.text : 'text-foreground'}`}>
                                    {p.nickname}
                        ...
                        <div className="flex items-center gap-4 text-sm">
                            <div className="w-16 text-right font-medium text-muted-foreground/80 tabular-nums">
