    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex items-center text-xs font-bold text-muted-foreground px-4 mb-2 uppercase tracking-wider opacity-70">
                ...
                <span className="w-20 text-center mr-2">
                    {mode === 'SPEED_RACE' ? 'Time' : 'Solved'}
                </span>
                ...
                {sortedParticipants.map((p) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {p.isMe && (
                                <div className={`absolute -inset-[1px] rounded-xl blur-[2px] opacity-70 animate-pulse ${glowColor}`} />
                            )}

                            <div className={`relative flex items-center py-4 px-5 rounded-xl border ${p.isMe
                                ? `bg-card ${borderColor}`
                            ...
                            >
                                ...
                                <div className="flex-1 flex items-center gap-4 ml-4 overflow-hidden">
                                    <div className={`w-10 h-10 rounded-full overflow-hidden border-2 ${p.rank === 1 ? 'border-yellow-500' : 'border-border'
                                        }`}>
                                        {p.profileImg ? (
                                            <Image src={p.profileImg} alt={p.nickname} width={40} height={40} className="object-cover" />
                                        ) : (
                                            <div className="w-full h-full bg-muted" />
                                        )}
                                    </div>
                                    <span className={`flex items-center text-lg font-bold truncate ${p.isMe ? 'text-yellow-400' : 'text-foreground'}`}>
                                        {/* Team Badge */}
                                        {teamType === 'TEAM' && p.teamId && (
                                            <span className={`mr-2 px-1.5 py-0.5 rounded text-[10px] font-black uppercase tracking-wider border ${p.teamId === 'RED'
                                ...
                                <div className={`w-20 text-center font-bold text-base mr-2 flex items-center justify-center ${p.isMe ? 'text-yellow-400/80' : 'text-muted-foreground'}`}>
                                    {mode === 'SPEED_RACE' ? (
                                ...
                                <div className="w-24 text-right shrink-0">
                                    <span className={`text-lg font-black tracking-tight ${p.isMe ? 'text-yellow-400' : 'text-muted-foreground'}`}>
