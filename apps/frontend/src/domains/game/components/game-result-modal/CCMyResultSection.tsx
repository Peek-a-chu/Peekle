...
const getRankDisplay = (rank: number) => {
    if (rank === 1) {
        return {
            icon: <Crown className="w-20 h-20 text-yellow-400 fill-yellow-400 drop-shadow-[0_0_25px_rgba(250,204,21,0.6)]" />,
            label: 'WINNER',
            subLabel: '#1',
...
export function CCMyResultSection({
    ...
    return (
        <div className="w-[40%] p-5 flex flex-col items-center justify-center border-r border-border/40 bg-card/30 relative overflow-hidden">
            {/* 1, 2, 3등일 때 배경 효과 */}
            {glowColor && (
                <div className={`absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] ${glowColor} via-transparent to-transparent animate-pulse`} />
            )}
            ...
            <div className="w-full flex flex-col items-center justify-center gap-5 relative z-10">
                ...
                <div className={`flex flex-col items-center gap-3 ${rankDisplay.containerClass}`}>
                    <div className="transform drop-shadow-2xl relative">
                        {rankDisplay.icon}
                    </div>
                    <div className="flex flex-col items-center leading-none">
                        <span className={`text-6xl font-black tracking-tighter drop-shadow-lg ${rankDisplay.textClass}`}>
                            {rankDisplay.label}
                        </span>
                        {myRank === 1 && (
