...
import { HelpCircle, ChevronUp, ChevronDown, CheckCircle2, Brain, Gamepad2, Users } from 'lucide-react';
import {
    Dialog,
    DialogContent,
    DialogHeader,
...
const LeagueRuleModal = ({ myLeague = 'gold', myPercentile, leagueStats }: LeagueRuleModalProps) => {
                ...
                <div className="flex h-[500px]"> {/* 높이 줄임 */}
                    ...
                    <div className="flex-1 flex flex-col min-w-0">
                        ...
                        <Tabs defaultValue="grade" className="flex-1 flex flex-col overflow-hidden">
                            ...
                            {/* 1. 승급/강등 탭 */}
                            <TabsContent value="grade" className="flex-1 p-6 pt-4 overflow-y-auto space-y-3 custom-scrollbar">
                                ...
                                {promotePercent > 0 ? (
                                    <div className="p-3.5 rounded-xl border border-emerald-500/20 bg-emerald-500/5 flex items-start gap-3">
                                        ...
                                        <div>
                                            <div className="text-sm font-bold text-emerald-600 dark:text-emerald-400 mb-0.5">승급 구간</div>
                                ...
                                {demotePercent > 0 ? (
                                    <div className="p-3.5 rounded-xl border border-red-500/20 bg-red-500/5 flex items-start gap-3">
                                        ...
                                        <div>
                                            <div className="text-sm font-bold text-red-600 dark:text-red-400 mb-0.5">강등 구간</div>
                                            <div className="text-xs text-muted-foreground">
                            ...
                            <TabsContent value="score" className="flex-1 p-6 pt-4 overflow-y-auto custom-scrollbar">
                                <div className="space-y-2">
                                    {SCORE_RULES.map((rule, idx) => (
                                        <div key={idx} className={`p-3 rounded-lg border transition-colors ${rule.style}`}>
                                            <div className="flex items-center justify-between mb-1">
                                                <div className="flex items-center gap-2.5">
                                                    <div className={`p-1.5 rounded-md border shadow-sm ${rule.iconStyle}`}>
                                                        <rule.icon className="w-4 h-4" />
                                                    </div>
                                                    <span className={`text-sm font-bold ${rule.textStyle}`}>{rule.label}</span>
                                                </div>
                                ...
                                <div className="group p-3 rounded-lg border border-indigo-500/20 bg-indigo-500/5 hover:bg-indigo-500/10 transition-colors mt-2">
                                    <div className="flex items-center justify-between mb-1">
                                        <div className="flex items-center gap-2.5">
                                            ...
                                            <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400">팀전 점수 분배</span>
                                        </div>
                                        <Badge variant="outline" className="font-mono bg-indigo-500/10 border-indigo-500/20 text-indigo-600 dark:text-indigo-400">
                                            균등 분배
                                        </Badge>
                                    </div>
                                    <p className="text-[11px] text-muted-foreground pl-10 leading-snug">
                                        승리 팀이 상위 순위(1~N/2등) 점수 합계를 가져가며 팀원들에게 <strong>균등 분배</strong>됩니다. <br className="hidden sm:block" />
