...
export default function LeagueResultModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#131F24] rounded-2xl w-full border border-white/5 text-center shadow-2xl overflow-hidden flex flex-col max-h-[80vh] max-w-sm animate-in fade-in zoom-in duration-300">
        {step === 'RANKING' ? (
          <RankingView
            history={history}
            ranking={ranking}
          ...
          <ResultView history={history} onContinue={handleContinue} />
        )}
      </div>
    </div>
  );
    ...
    <>
      <div className="bg-card p-6 pb-2">
        <div className="flex justify-center mb-4">
          <LeagueIcon
            league={history.league.toLowerCase() as LeagueType}
            size={64}
...
function RankingItem({ member }: { member: BackendRankingMember }) {
  ...
  return (
    ...
    >
      <div
        className={`w-8 font-bold text-center ${isMe ? 'text-primary' : 'text-muted-foreground'}`}
      >
        {member.rank}
      </div>

      <div className="mx-3 relative">
        <UserIcon src={member.profileImgThumb} nickname={member.name} size={40} />
        {isMe && (
      ...
      <div className="flex-1 text-left min-w-0">
        ...
        <div className="text-xs text-muted-foreground">{member.score.toLocaleString()} XP</div>
      </div>
    </div>
  );
}
...
function ResultView({
  ...
  const targetLeague =
    (history.currentLeague?.toLowerCase() as LeagueType) ||
    (history.league.toLowerCase() as LeagueType);
  ...
  return (
    <div className="flex flex-col h-full bg-card p-6">
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <h2 className="text-xl font-bold text-muted-foreground mb-1">{title}</h2>
        <h1 className="text-2xl font-extrabold text-card-foreground mb-6">{subTitle}</h1>
      </div>
