  ...
  return (
    ...
    <div className="min-h-screen bg-background font-sans selection:bg-secondary selection:text-primary flex flex-col">
      ...
      <header className="sticky top-0 w-full flex justify-between items-center px-6 py-4 bg-background/80 backdrop-blur-md z-50 border-b border-border max-w-7xl mx-auto w-full">
        ...
        <div>
            <Link href="/login">
                <Button className="font-bold rounded-md px-6">
                    로그인
                </Button>
            </Link>
        </div>
      ...
      <main className="flex-1 w-full">
        ...
        <section className="flex flex-col items-center justify-center text-center pt-24 pb-32 px-4 max-w-5xl mx-auto">
          ...
          <p className="text-lg md:text-xl text-[#6B7280] max-w-2xl mb-10 leading-relaxed font-medium break-keep">
            실시간 화상 스터디와 경쟁적인 게임 모드로<br className="hidden md:block"/> 알고리즘 실력을 빠르게 향상시키세요.
          </p>
          <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/login">
                  <Button size="lg" className="text-lg px-8 py-6 rounded-md shadow-lg shadow-primary/20 hover:shadow-xl hover:-translate-y-1 transition-all flex items-center gap-2 font-bold">
        ...
        <section className="py-24 px-4 bg-background">
          <div className="max-w-6xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                      { icon: Users, title: "실시간 스터디", desc: "화상으로 함께 문제를 풀고 코드를 공유하세요" },
                      { icon: Gamepad2, title: "경쟁 게임 모드", desc: "스피드 레이스, 타임어택 등 다양한 게임 모드로 실력을 겨뤄보세요." },
                      { icon: Trophy, title: "리그 시스템", desc: "매주 경쟁하고 승급하여 실력을 증명하세요" },
                      { icon: Target, title: "AI 문제 추천", desc: "나에게 딱 맞는 문제를 AI가 추천해드려요" }
                  ].map((item, idx) => (
                      // 카드: bg-card (#FFFFFF), border-card-border (#F7E8F0) 사용
                      <div key={idx} className="bg-card p-8 rounded-2xl border border-card-border shadow-sm hover:shadow-md transition-all duration-300">
                          ...
                          <p className="text-[#6B7280] text-sm leading-relaxed break-keep">
                              {item.desc}
                          </p>
                      </div>
                  ))}
          ...
          <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12 text-center px-4">
              {[
                  ...
                  <div key={idx} className="flex flex-col items-center w-full">
                      <span className="text-4xl md:text-5xl font-extrabold text-primary mb-2">{stat.num}</span>
        ...
        <section className="bg-primary py-32 text-center px-4 relative overflow-hidden">
          <div className="relative z-10 max-w-3xl mx-auto">
              ...
              <p className="text-white/90 mb-10 text-lg md:text-xl font-medium">무료로 가입하고 알고리즘 실력을 키워보세요.</p>
              <Link href="/login">
                  {/* 흰색 버튼에 검은 글씨 (시안 반영) */}
                  <Button size="lg" className="bg-white text-foreground hover:bg-gray-100 text-lg px-8 py-6 rounded-md font-bold shadow-2xl transition-all">
