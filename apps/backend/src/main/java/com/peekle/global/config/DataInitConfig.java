		...
		return args -> {
			...
			if (studyProblemRepository.count() == 0) {
				StudyRoom room = studyRoomRepository.findAll().stream()
						.filter(r -> r.getTitle().equals("Morning Algorithm"))
						.findFirst()
						.orElse(studyRoomRepository.findAll().get(0));
				User owner = userRepository.findByNickname("AlgorithmKing").orElse(userRepository.findAll().get(0));
			...
			if (studyChatLogRepository.count() == 0) {
				StudyRoom room = studyRoomRepository.findAll().stream()
						...
						.orElse(studyRoomRepository.findAll().get(0));
				User owner = userRepository.findByNickname("AlgorithmKing").orElse(userRepository.findAll().get(0));
				User member1 = userRepository.findByNickname("NewbieCoder").orElse(userRepository.findAll().get(1));
