...
public interface SubmissionLogRepository extends JpaRepository<SubmissionLog, Long>, SubmissionLogRepositoryCustom {
        ...
        // [New] 특정 유저의 총 *성공한* 문제 수 조회 (중복 제거, isSuccess = true)
        @org.springframework.data.jpa.repository.Query("SELECT COUNT(DISTINCT s.problem.id) FROM SubmissionLog s WHERE s.user.id = :userId AND s.isSuccess = true")
