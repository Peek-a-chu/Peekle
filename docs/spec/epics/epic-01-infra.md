...
서비스 운영자 및 개발자로서, 애플리케이션의 빌드 속도와 런타임 성능을 전 계층(Frontend/Backend/Infra)에서 최적화하고 싶다.

사용자에게 빠른 응답 속도를 제공하고, 배포 효율성을 극대화하여 비용을 절감하기 위함이다.

✅ Acceptance Criteria

Frontend 빌드 시간이 기존 대비 50% 이상 단축되어야 한다 (Turbo Prune 적용).

Backend API 응답 지연이 최소화되어야 하며, 동시 접속 처리가 효율적이어야 한다 (Virtual Threads, ZGC).

Docker 이미지 빌드 시 불필요한 컨텍스트 전송이 없어야 한다.

CI/CD 파이프라인에서 변경된 모듈만 지능적으로 빌드해야 한다.

리버스 프록시 레벨에서 SSL 캐싱 및 압축이 적용되어야 한다.

**🛠 Implementation Tasks**
[x] Frontend Dockerfile 최적화 (Turbo Prune, Standalone Mode)
[x] Frontend `next.config.ts` 최적화 (Tree Shaking, Bundle Analyzer)
[x] Backend JVM 옵션 튜닝 (ZGC, GC Logging) 및 Virtual Threads 활성화
[x] Gradle 빌드 캐싱 및 병렬 처리 설정 (`gradle.properties`)
[x] Docker 빌드 컨텍스트 최소화 (`.dockerignore`)
[x] Nginx SSL 성능 튜닝 및 Gzip 압축 설정
[x] GitLab CI Smart Diff Build 로직 구현
