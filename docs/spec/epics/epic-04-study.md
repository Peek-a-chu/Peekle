...
### S5-12. 스터디 문제 해결 인증 (Extension)

🧾User Story

스터디원으로서, 스터디 과제로 등록된 문제를 풀면 자동으로 인증하고 싶다.

일일이 캡처하거나 링크를 올리는 번거로움 없이 스터디 진행률을 갱신하기 위함이다.

✅ Acceptance Criteria

[컨텍스트 주입] 스터디 방 입장 혹은 "문제 풀러 가기" 클릭 시 `studyId`가 확장 프로그램에 저장되어야 한다.

[결과 전송] 정답 처리 시 `POST /api/study/{studyId}/submit`을 호출하여 문제 해결을 알린다.

[스터디 현황 반영] 제출 완료 시 스터디 캘린더나 문제 목록에 "해결됨(Solved)" 상태가 공유되어야 한다.

[코드 저장] 제출한 코드는 스터디원들이 볼 수 있도록 서버에 저장되어야 한다.

**🛠 Implementation Tasks**
[ ] (Extension) 스터디 컨텍스트(`type: 'STUDY', studyId`) 저장 로직 활용
[ ] 스터디 제출 처리 API 구현 (문제 해결 상태 업데이트 및 코드 저장)
[ ] 제출된 코드 저장 및 '남의 코드 보기' 기능과 연동
