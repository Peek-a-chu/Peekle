요청하신 대로 **UNDO(실행 취소) 기능을 완전히 제거**하고, 앞서 논의된 **Toast 알림** 및 **객체 단위 삭제/수정 로직**을 반영하여 완성된 상세 명세서입니다.
...
2. **알림**: 화면 우측 하단에 **Toast 알림** 메시지 표시
...
* **클릭**: Toast 클릭 시 화이트보드 오버레이 즉시 오픈

### 화이트보드 오버레이 (IDE 영역)

**UNDO 버튼이 제거된 최종 UI**입니다.
...
<br>(Client  Server) | **PUB** | `/pub/studies/whiteboard/message` | `json { "action": "ADDED", "objectId": "uuid-123", "data": {...} } ` |
...
<br>(Server  Client) | **SUB** | *(위와 동일)* | `json { "action": "ADDED", "objectId": "uuid-123", "senderId": 100, "data": { "type": "rect", ... } } ` |
| **초기 동기화 수신**<br>

<br>(입장 시) | **SUB** | `.../whiteboard/{uid}` | `json { "action": "SYNC", "data": { "isActive": true, "history": [{ "action": "ADDED", ... }] } } ` |
...
| **`CURSOR`** | 커서 위치 공유 | `x`, `y` | **Redis 저장 X**, 실시간 브로드캐스팅만 수행 |
...
프론트엔드에서 도구 사용 시 발생시켜야 할 이벤트와 처리 로직입니다. (UNDO 제외됨)

| 도구 (Tool) | 동작 시점 (Trigger) | 전송 Action | Payload 예시 | 비고 (Implementation Note) |
| --- | --- | --- | --- | --- |
| **✏️ 펜**<br>

<br>(Free Drawing) | `path:created`<br>

<br>(마우스 뗄 때) | **ADDED** | `data: { type: "path", path: [...], stroke: "red" ... }` | Fabric.js의 `Path` 객체를 JSON으로 변환하여 전송 |
| **🔲 도형**<br>

<br>(Shape) | `mouse:up`<br>

<br>(드래그 완료 시) | **ADDED** | `data: { type: "rect", width: 100, height: 100 ... }` | 드래그 중(`mouse:move`)에는 로컬에서만 그리고, 완료 시 서버 전송 |
| **T 텍스트**<br>

<br>(Text) | `text:editing:exited`<br>

<br>(입력 완료 시) | **ADDED**<br>

<br>or<br>

<br>**MODIFIED** | `data: { type: "i-text", text: "Hello", ... }` | 최초 생성 시 `ADDED`, 내용 수정 시 `MODIFIED` 전송 |
| **🗑 지우개**<br>

<br>(Eraser) | `mouse:down`<br>

<br>(객체 클릭 시) | **REMOVED** | `objectId: "uuid-123"`<br>

<br>`data: null` | **객체 단위 삭제 방식**<br>

<br>클릭된 객체의 `objectId`를 식별하여 삭제 요청 전송 |
| **✋ 선택/이동**<br>

<br>(Select/Move) | `object:modified`<br>

<br>(변형 완료 시) | **MODIFIED** | `objectId: "uuid-123"`<br>

<br>`data: { left: 200, top: 50 ... }` | 이동/크기조절/회전 후 마우스를 뗄 때 최종 상태 전송 |
...
| 상태 (State) | 설명 |
| --- | --- |
| `isWhiteboardActive` | 화이트보드 세션 활성화 여부 (방 전체 공유 상태) |
| `whiteboardOpenedBy` | 화이트보드를 처음 연 사람 정보 |
| `isOverlayOpen` | 현재 사용자의 오버레이 UI 표시 여부 (로컬 상태) |
| `drawingData` | 실시간 동기화되는 드로잉 객체 Map (`objectId`를 Key로 사용) |
| `peersCursors` | 다른 참여자들의 마우스 커서 위치 Map |
...
* [x] **UI Component**: 화이트보드 오버레이 모달 UI 구현 (UNDO 버튼 제외)
...
* [x] **Optimization**: 마우스 커서(`CURSOR`) 이벤트 스로틀링(Throttling) 적용